...
export class AdminService {
  ...
  async login(email: string, password: string): Promise<AdminLoginResponseDto> {
    ...
    const payload: Omit<AdminJwtPayload, never> = {
      sub: adminUser.id,
    ...
    const accessToken = this.jwtService.sign(payload, {
      ...
      expiresIn: (this.config.get<string>('jwtAdminExpiresIn') || '8h') as any,
    });
  ...
  async deleteImage(imageId: string, admin: AdminJwtPayload): Promise<AdminDeleteResponseDto> {
    ...
    const client = await this.prisma.client.findUnique({ where: { id: image.clientId } });
  ...
  async deleteAvatar(avatarId: string, admin: AdminJwtPayload): Promise<AdminDeleteResponseDto> {
    ...
    const client = await this.prisma.client.findUnique({ where: { id: avatar.clientId } });
    const domain = client?.domain || avatar.clientId;
    const avatarPath = this.storage.getAvatarPath(domain, avatar.userId);
