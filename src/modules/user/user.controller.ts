...
export class UserClientController {
  ...
  @Get()
  @ApiOperation({
    summary: 'List users for this client (email is never returned)',
  })
  ...
  listUsers(
    ...
    return this.userService.listUsersForClient(clientId, {
      search,
      page,
      perPage,
    });
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid payload or reserved externalUserId',
  })
  @ApiResponse({
    status: 401,
    description: 'Missing or invalid X-API-Key header',
  ...
  @ApiResponse({
    status: 409,
    description: 'User with that externalUserId already exists',
  })
  createUser(
    @ClientId() clientId: string,
  ...
  updateUserByExternalUserId(
    ...
    return this.userService.updateUserByExternalUserId(
      clientId,
      externalUserId,
      dto,
    );
